...
# Stripe route to create a checkout session
...
def payment_success():
    ...
    user_id = create_user_account(session_data)

    return f'Payment successful! User created with ID: {user_id}'
...
# Function to create a user account after payment success
def create_user_account(session_data):
    user_id = generate_unique_id()
    # Here, save the user details to your database or session
    session['user_id'] = user_id
    session['outputs_this_month'] = 0
    return user_id
...
def stripe_webhook():
    ...
    try:
        event = stripe.Webhook.construct_event(
            payload, sig_header, 'your-webhook-signing-secret'
        )
    ...
    if event['type'] == 'checkout.session.completed':
        ...
        create_user_account(session_data)

    return '', 200

@app.route('/process', methods=['POST'])
def process():
    # User management - check output limit
    user_id = session.get('user_id')
    if not user_id:
        return redirect(url_for('index'))

    outputs_this_month = session.get('outputs_this_month', 0)
    if outputs_this_month >= MAX_OUTPUTS_PER_MONTH:
        return 'Output limit reached for the month', 403
    ...
    # Increment the output count after processing
    session['outputs_this_month'] = outputs_this_month + 1
